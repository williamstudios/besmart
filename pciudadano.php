    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content mt-3">
          <div class="col-md-4">
            ...
            <select class="form-control" name="" id="concepto">
              ...
              <?php
              $depas = mysqli_query($con, "SELECT * FROM personas WHERE id_cliente = '$id_cliente' ORDER BY nombre ASC");
              while ($rdepas = mysqli_fetch_assoc($depas)) {
                echo '<option value="' . $rdepas["nombre"] .'">' . $rdepas["nombre"] .'</option>';
              }
              ?>
            </select>
          </div>
        ...
        <script type="text/javascript">
          function busca(){
            ...
            var url = "reporteciudadano.php?concepto=" + concepto + "&tipo=" + tipo; // El script a dónde se realizará la petición.
            $.ajax({
               url: url,
               beforeSend: function () {
                 $(".tablamain").html("");
