...
<table id="tabla" class="tabla table table-striped table-bordered">
  ...
  <tbody>
    ...
    if ($concepto == "Todos") {
      ...
      if ($tipo == "todos") {
        $busca = mysqli_query($con, "SELECT pet.*, depa.Nombre AS depar FROM peticiones AS pet INNER JOIN Departamentos AS depa ON pet.departamento_asignado = depa.id WHERE pet.id_cliente = '$id_cliente' AND pet.usuario_asignado = '$concepto' ORDER BY pet.id DESC LIMIT 500");
      } else {
        $busca = mysqli_query($con, "SELECT pet.*, depa.Nombre AS depar FROM peticiones AS pet INNER JOIN Departamentos AS depa ON pet.departamento_asignado = depa.id WHERE pet.id_cliente = '$id_cliente' AND pet.usuario_asignado = '$concepto' AND pet.status = '$tipo' ORDER BY pet.id DESC LIMIT 500");
      }
    }
